...
export default{
    ...
    actions:{
        ...
        //添加或修改
        saveOrUpdate(context,driver){
            
            return post('/driver/saveOrUpdate',driver)
            .then((result)=>{
                //实现提示消息在状态机中
                // this._vm.$message({
        ...
        deleteById(context,id){
            return get('/driver/deleteById',{id})
                .then((result)=>{
                    context.commit('resetMsg',result.statusText);
                    context.dispatch('query');
                })
        ...
        batchDelete({commit,dispatch},ids){
            post_array('/driver/batchDelete',{ids})
            .then((result)=>{
                //注意与上边的区别
        ...
        nameQuery(context,{realname}){
            context.commit('resetLoading',true)
            post('/driver/mohuQuery',{realname})
            .then((result)=>{
                context.commit('resetList',result.data);
            })
            .finally(()=>{
