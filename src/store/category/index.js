...
export default{
    ...
    actions:{
        ...
        //按照姓名查找
        nameQuery(context,{address}){
            context.commit('resetLoading',true)
            post('/address/mohuQuery',{
                address
            })
            .then((result)=>{
                context.commit('resetList',result.data);
        ...
        //添加或修改
        saveOrUpdate(context,address){
            
            return post('/address/saveOrUpdate',address)
            .then((result)=>{
                //实现提示消息在状态机中
                // this._vm.$message({
        ...
        deleteById(context,id){
            return get('/address/deleteById',{id})
                .then((result)=>{
                    context.commit('resetMsg',result.statusText);
                    context.dispatch('query');
                })
        ...
        batchDelete({commit,dispatch},ids){
            post_array('/address/batchDelete',{ids})
            .then((result)=>{
                //注意与上边的区别
